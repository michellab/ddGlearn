...
def build_reactions(perturbations_all_paths, MorphFilePath):
    ...
    # regenerate the perturbation (A>B):
        ...
        ligA = perturbation_pair_path[0].replace(path_to_mols, "").replace("/ligand.mol","")
        ligB = perturbation_pair_path[1].replace(path_to_mols, "").replace("/ligand.mol","")
        perturbation = str(ligA) + ">" + str(ligB)

    # read in mol files:
        perturbation_pair = []

        perturbation_pair.append(rdmolfiles.MolFromMolFile(perturbation_pair_path[0]))
        perturbation_pair.append(rdmolfiles.MolFromMolFile(perturbation_pair_path[1]))
